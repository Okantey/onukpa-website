...
import { submitPropertyWithToken, type PropertyPayload } from "../../api/suppliers";

type CategoryKey = "room" | "apartment" | "hostel" | "office";
...
> = {
  ...
  hostel: { label: "Hostel", icon: GraduationCap },
  office: { label: "Office / Store Space", icon: Store },
};
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload: PropertyPayload = {
        ...
        monthlyEstimate: Number(monthlyEstimate),
        advanceMonths: advanceMonths ? Number(advanceMonths) : undefined,
        totalPayableNow: totalPayableNow ? Number(totalPayableNow) : undefined,
        availabilityStatus,
          ...
          {submitted ? (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8 text-center space-y-4">
              ...
              <p className="text-sm text-slate-500">
                You can use this same link again to add another property later.
              </p>
            </div>
          ) : (
            <form
            ...
            >
              ...
              {/* Placeholder for category-specific attributes; can be expanded later */}
              <div className="text-xs text-slate-500">
                After we connect this form to the full backend, we can add richer
                fields per category (washrooms, parking, beds, size, etc.) based on
                Onukpa’s property model.
              </div>
